...
class lifecycle_iterate:
    ...
    def __init__(self, paramSub, file_path='share.f90', new=False, sub=True):
        ...
        for file in (['vfunc1', 'vfunc2', 'vfunc3', 'vfunc4', 'model_log',
                      'policy_constantwealth', 'transition_lagsleads',
    ...
    def execSh(self, thread=40, **kwargs):
        ...
        print ('################################################# \n'
               ...
               '#################################################')
        proc = Popen([self.mainD + 'lifecycle_build.sh', '%d' % thread],
    ...
    def matlabPlot(self, life=40, pol='FTHB', **kwargs):
        ...
        try:
            for name in kwargs['model']:
                ...
                copyfile(self.dir + 'FTHB_dist_data.csv',
