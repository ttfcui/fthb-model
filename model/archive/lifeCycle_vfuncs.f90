...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    REAL(8) FUNCTION temp_expect_loop(EVmat, aval, tmpnodes, probtmp, weightDprimel, &
        ...
        !write(*,*) EVmat(zindex, 1, aprimel_vec(1):aprimeh_vec(1))
        !DEALLOCATE(weightaprimel, aprimeh_vec, aprimel_vec)
        !block
        if (present(transfers)) then
            do i=1, tmpgridsize
                aprime_shift(i) = asset_shift(aval, pwealth, &
                                              transfers + tmpnodes(i))
                !write(*,*) aval, aprime_shift(i)
            end do
        else
            do i=1, tmpgridsize
                aprime_shift(i) = asset_shift(aval, pwealth,tmpnodes(i))
        ...
        call weightPrimevec(aprime_shift, anodes, weightaprimel, &
             aprimel_vec, aprimeh_vec)
         !write(*,*) EVmat(zindex, 1, aprimel_vec(1):aprimeh_vec(1)) !block 
    ...
    REAL(8) FUNCTION expectation(zindex, Dval, aval, tval, hpval, price, uval, pwealth, Down,&
        ...
        null_vec = (/ 0.0, 1.0 /)
        tmpgrid => null_vec(1:1)
        probtmp => null_vec(2:2)
        if (rental) then
            weightDprimel = 0.0
            Dprimel = 1
            Dprimeh = Dprimel
    ...
    REAL(8) FUNCTION valfuncadjust(p, state, pol_opts)
        ...
        if (aprime>=borrowconstraint .AND. Dprime>=Dmin) then
            ...
            if (consumption > 0) then
                ...
                if (zindex == 10 .AND. D == 0.0 .AND. t == 2 .AND. 0.2 < a .AND. a < 0.3 .AND. transferfuture > 0.0) then
                    !write(*, *) transferfuture
                    !write(*, *) downflag*(1.0-discountflag)*(1.0-exp(-transfer_down))*Dprime
                end if
                !write(*,*) "============================"
               !write(*,*), expectation(INT(zindex), (1-delta+dep+discountflag*transfer_down)*Dprime,&
               !                      aprime, t+1, hpindex+1, exp(hpholder), unemp, futliquid, D,&
               !                      pol_opts(2), .FALSE.,.TRUE.,0.0)

               !write(*,*), expectation(INT(zindex), (1-delta+dep+discountflag*transfer_down)*Dprime,&
               !                      aprime, t+1, hpindex+1, exp(hpholder), unemp, futliquid, D,&
               !                      pol_opts(2), .FALSE.,.TRUE.,0.5)
