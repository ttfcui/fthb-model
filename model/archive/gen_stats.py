    ...
    for num in xrange(0, 3):
        try:
            all_stats.append(cleaning(grouped['id'].count().loc[idx[num, 1:]],
                                      'Extensive from %d' % num,
                                      'id', 'adjustment_pol'))
            all_stats.append(cleaning(grouped_a['id'].count().loc[idx[num, 1:,
                                      :]], 'Extensive from %d' % num,
                                      'id', 'adjustment_pol'))
        except:
            all_stats.append(cleaning([0], 'Extensive from %d' % num, 0))

    # TODO: May have to put try statements around agebin collapses,
    # because will fail out due to groupby clause
    all_stats.append(cleaning([grpSlice.loc[grpSlice['diff'] > 0,
                     'diff'].sum()], 'More transactions over lifecycle', 0))
    all_stats.append(cleaning(grpASlice.loc[grpASlice['diff'] > 0,
                              ['diff', 'agebin']].groupby('agebin').sum(),
                              'More transactions over lifecycle', 'diff'))

    all_stats.append(cleaning([-grpSlice.loc[grpSlice['diff'] < 0,
                     'diff'].sum()], 'Fewer transactions over lifecycle', 0))
    all_stats.append(cleaning(-grpASlice.loc[grpASlice['diff'] < 0,
                              ['diff', 'agebin']].groupby('agebin').sum(),
                              'Fewer transactions over lifecycle', 'diff'))
...
def stats2(mdir='model', tretire=39, end=39):
    ...
    all_stats.append(cleaning(statFunc(data.loc[data[tretire] < 0, ivar]),
                              'All agents w/ fewer transactions + mean income',
                              ivar, 'index'))
    all_stats.append(cleaning(statGrpFunc(data.loc[data[tretire] < 0]
                                          .groupby('agebin')[ivar]),
                              'All agents w/ fewer transactions + mean income',
                              ivar, 'level_1'))
    all_stats.append(cleaning(statFunc(data.loc[data[tretire] > 0, ivar]),
                              'All agents w/ more transactions + mean income',
                              ivar, 'index'))
    all_stats.append(cleaning(statGrpFunc(data.loc[data[tretire] < 0]
                                          .groupby('agebin')[ivar]),
                              'All agents w/ more transactions + mean income',
