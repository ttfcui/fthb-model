...
module share
    ...
    REAL(8), parameter :: thetamatlab=.40
...
REAL(8), dimension(:,:), ALLOCATABLE :: housingstock, rentalstock
...
durableinvestment(numhouseholds,Tdie),actualwealth(numhouseholds,Tdie),financialwealth(numhouseholds,Tdie), &
...
ALLOCATE(alive(numhouseholds,Tdie),housingstock(numhouseholds,Tdie),rentalstock(numhouseholds,Tdie))
...
!$OMP PARALLEL DO
    do i=1,numhouseholds
        ...
        if ((rentalind(i,t)==0) .and. (t >= fthb_flag(i, 5) + 3) .and. &
             (t <= Tretire)) then
        ...
        if ((rentalind(i,t)==0) .and. (fthb_flag(i, 1) == Tdie+1)) then
            ...
            write(1989, 1989) i, t, fthb_flag(i, 2), fthb_flag(i, 3), fthb_flag(i, 4), -1
        end if
        ...
        ! Back to renting. Begin FTHB tracker so long as agent has bought house.
