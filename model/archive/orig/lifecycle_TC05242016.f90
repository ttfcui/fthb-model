!***********************************************************
!    This file is a generalized version of 04032016 because
!    it allows for separate, simultaneuous or staggered
!    simulation of buyback policies along with FTHB policies.
!    State variables track an agent's renting time or time
!    with the same durable good and uses those to determine
!    program eligibility.
...
module share
    ...
    REAL(8), parameter :: amax=6  !max asset

    integer, parameter :: agridsize = 80
    integer, parameter :: Dgridsize = 60
    ...
    integer, parameter :: zgridsize=25  !size of ar(1) income shock grid
    REAL(8), parameter :: borrowconstraint=0.0

    !House price subsidy shocks and policy settings
    REAL(8), parameter :: hpmin=-0.10
    ...
    !Potential anticipated shocks
    ! Total number of periods housing slump lasts, ending with policy enactment.
    ! Not the same as earlier definition of "number of housing slump models" 
    integer, PARAMETER :: totalbubblelength=2 
    REAL(8), DIMENSION(totalbubblelength) :: gd ! Vary elements for discrete jumps
    REAL(8), PARAMETER :: rho=0.5 ! Vary for price drift
    ...
    REAL(8), DIMENSION(totalbubblelength+2) :: P_exp
    ...
    REAL(8), dimension(numhouseholds, Tdie, 12) :: householdresultsholder
...
do while (diffmoments>difftol)
    ...
    ! Discrete jumps in housing bust price drops
    write(*,*) "Housing bust jumps:"
    gd(1)=0
    gd(totalbubblelength-1)=0
    gd(totalbubblelength)=0
...
1989 format (I6.2, I6.2, F16.6, F16.6, F16.6, I6.2)
...
currenthouseholdstate(:, 4)=hpgridsize  !high house price state
    ...
    do i=1, numhouseholds
        ...
        !  was taken since Matlab doesn't handle mixed types well)
                    if (fthb_flag(i, 1) == Tdie+1) then
                        ...
                        currenthouseholdstate(i, 1) - currenthouseholdstate(i, 2)*(1-theta), &
                        income(currenthouseholdstate(i, 3), t), -1
                    else if (fthb_flag(i, 3) == 0) then
                        fthb_flag(i, 1) = t
                        housingstock(i, t)=newhouseholdstate(i, 2)
                        write(1989, 1989) i, t, newhouseholdstate(i, 2), &
                        currenthouseholdstate(i, 1) - currenthouseholdstate(i, 2)*(1-theta), &
                        income(currenthouseholdstate(i, 3), t), -2
                    end if
                    fthb_flag(i, 3) = t
                end if
        end if
...
299 format (I6.2, I6.2, I6.2, F16.6, F16.6, F16.6, I6.2)
...
rentalflow(numhouseholds, Tdie, Tdie))
...
!$OMP PARALLEL DO
    do i=1, numhouseholds
        ...
        qchg(i, tj) = currenthouseholdstate(i, 1) + (1-thetastate(i))*currenthouseholdstate(i, 2)*hpdelta_i(i)
        ...
        ! Start the repeat buyer policy countdown whenever a durables purchase is made
            ...
            !  was taken since Matlab doesn't handle mixed types well)
                if ((fthb_flag(i, 1) == Tdie+1) .and. (tj <= Tretire)) then
                    ...
                    currenthouseholdstate(i, 1) - currenthouseholdstate(i, 2)*(1-thetastate(i)), &
                    incomeholder(i, tj), -1
...
ALLOCATE(ax(agridsize, Dgridsize, zgridsize, hpgridsize), bx(agridsize, Dgridsize, zgridsize, hpgridsize), cx(agridsize, Dgridsize, zgridsize, hpgridsize), adjust(agridsize, Dgridsize, zgridsize, hpgridsize))
    ...
    adjust=0
    P_exp=0 ! Value function consistency

    do t=Tdie, 1,-1
        age = t ! Value function consistency
...
subroutine solveanticipationproblem
    USE nrtype; USE nrutil
    USE nr
    USE share
    USE OMP_LIB
    IMPLICIT NONE
    INTEGER :: i,j,k,m,t,age,k2 
    REAL(8), dimension(agridsize, Dgridsize, zgridsize, hpgridsize, 3, 2) :: pstartadjust
    REAL(8), dimension(agridsize, Dgridsize, zgridsize, hpgridsize, 3) :: ystartadjust
    REAL(8), dimension(:,:,:,:), ALLOCATABLE :: ax, bx, cx, adjust
    REAL(8) :: apol, Dpol
    REAL(8), dimension(agridsize, Dgridsize, zgridsize, hpgridsize, 5) :: state
    REAL(8), DIMENSION(:,:,:,:,:), ALLOCATABLE :: Velig, Vinelig ! Value of adjustment with the credit, only if eligible
    REAL(8), DIMENSION(:,:,:,:,:), ALLOCATABLE :: achoiceinelig, Dchoiceinelig, cchoiceinelig
    REAL(8), DIMENSION(:,:,:,:,:), ALLOCATABLE :: EVelig, EVinelig ! Different continuation values conditional on FTHB eligibility
    ...
    P1 = 1
    do t=1, totalbubblelength+1
       P_exp(t)=P1*exp(gd(t)*(1-rho**(t-1))/(1-rho)) ! t-1 because present price level is 1
    end do
    P_exp(totalbubblelength+2) = 1
    ...
    do t=age+totalbubblelength,age,-1
    write(*,*) t-age, age
    ...
    !$OMP DO
        do i=1, agridsize
            do j=1, Dgridsize
                do k=1, zgridsize
                    do l=hpgridsize,hpgridsize
                        if (t-age == totalbubblelength) then

                        P_exp(t-age+1) = P_exp(t-age+1)*exp(hpnodes(1))
                        ...
                        call amoeba(state(i, j, k, l,:), pstartadjust(i, j, k, l,:,:), ystartadjust(i, j, k, l,:), ftol, valfuncadjust)
                        if (ystartadjust(i, j, k, l, 1)<Vadjelig(i, j, k, l, t)) then  !(again we're minimizing)
                            Vadjust(i, j, k, l, t)=ystartadjust(i, j, k, l, 1)
                            achoiceadjust(i, j, k, l, t)=pstartadjust(i, j, k, l, 1, 1)
                            Dchoiceadjust(i, j, k, l, t)=pstartadjust(i, j, k, l, 1, 2)
                        end if
                        ...
                        call amoeba(state(i, j, k, l,:), pstartadjust(i, j, k, l,:,:), ystartadjust(i, j, k, l,:), ftol, valfuncadjust)
                        Vinelig(i, j, k, l, t)=ystartadjust(i, j, k, l, 1)
                        achoiceinelig(i, j, k, l, t)=pstartadjust(i, j, k, l, 1, 1)
                        Dchoiceinelig(i, j, k, l, t)=pstartadjust(i, j, k, l, 1, 2)
                        ...
                        if (anodes(i)-(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)<0) then
                                cchoiceinelig(i, j, k, l, t)=income(k, t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoiceadjinelig(i, j, k, l, t)*P_exp(t-age+1)-achoiceadjinelig(i, j, k, l, t)-(1+rborrow)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                        else
                                cchoiceinelig(i, j, k, l, t)=income(k, t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoicenoadjinelig(i, j, k, l, t)*P_exp(t-age+1)-achoicenoadjinelig(i, j, k, l, t)-(1+r)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                        end if
                        ...
                        Velig(i, j, k, l, t)=Vrent(i, j, k, l, 1)
                        ...
                        if (anodes(i)-(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)<0) then
                                cchoiceinelig(i, j, k, l, t)=income(k, t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoiceinelig(i, j, k, l, t)*P_exp(t-age+1)-achoiceinelig(i, j, k, l, t)-(1+rborrow)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                        else
                                cchoiceinelig(i, j, k, l, t)=income(k, t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoicenoinelig(i, j, k, l, t)*P_exp(t-age+1)-achoicenoinelig(i, j, k, l, t)-(1+r)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                        end if
                        ...
                        if (costlyequity==1) then
                            ...
                            if (anodes(i)>=(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)) then
                                ax(i, j, k, l)=(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                            else
                                ax(i, j, k, l)=anodes(i)
                            end if
                        bx(i, j, k, l)=ax(i, j, k, l)
                        ...
                        ! Gideon: computing cchoiceadjust, cchoicenoadjust and cchoicerent
                        if (anodes(i)-(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)<0) then
                                cchoiceadjust(i, j, k, l, t)=income(k, t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoiceadjust(i, j, k, l, t)*P_exp(t-age+1)-achoiceadjust(i, j, k, l, t)-(1+rborrow)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                                cchoicenoadjust(i, j, k, l, t)=income(k, t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*P_exp(t-age+1)-thetanodes(l)*Dchoicenoadjust(i, j, k, l, t)*P_exp(t-age+1)-achoicenoadjust(i, j, k, l, t)-(1+rborrow)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                                cchoicerent(i, j, k, l, t)=income(k, t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-r_rental*Dchoicerent(i, j, k, l, t)*exp(rentelasticity*hpnodes(l))-achoicerent(i, j, k, l, t)-(1+rborrow)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                        else
                                cchoiceadjust(i, j, k, l, t)=income(k, t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoicenoadjust(i, j, k, l, t)*P_exp(t-age+1)-achoicenoadjust(i, j, k, l, t)-(1+r)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                                cchoicenoadjust(i, j, k, l, t)=income(k, t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*P_exp(t-age+1)-thetanodes(l)*Dchoicenoadjust(i, j, k, l, t)*P_exp(t-age+1)-achoicenoadjust(i, j, k, l, t)-(1+r)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                                cchoicerent(i, j, k, l, t)=income(k, t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-r_rental*Dchoicerent(i, j, k, l, t)*exp(rentelasticity*hpnodes(l))-achoicerent(i, j, k, l, t)-(1+r)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                        end if
                        ...
                        if (Vadjust(i, j, k, l, t)<Vnoadjust(i, j, k, l, t) .and. Vadjust(i, j, k, l, t)<Vrent(i, j, k, l, t)) then  ! since V = - V from minimization
                            ...
                            Dchoice(i, j, k, l, t)=Dchoiceadjust(i, j, k, l, t)
                            if (anodes(i)-(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)<0) then
                                cchoice(i, j, k, l, t)=income(k, t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+rborrow)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                                !write(*,*) "diff1", income(k, t)+(1+rborrow*P_exp(t-age+1)**)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+rborrow*P_exp(t-age+1)**)*(1-thetanodes(l))*(1-F)*Dnodes(j)*P_exp(t-age+1)-(income(k, t)+(1+rborrow*P_exp(t-age+1)**(-0.0))*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+rborrow*P_exp(t-age+1)**(-0.0))*(1-thetanodes(l))*(1-F)*Dnodes(j)*P_exp(t-age+1))
                            else
                                cchoice(i, j, k, l, t)=income(k, t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-thetanodes(l)*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+r)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                            end if
                            rentalindicator(i, j, k, l, t)=0
                            choiceindicator(i, j, k, l, t)=1
                        elseif (Vnoadjust(i, j, k, l, t)<Vadjust(i, j, k, l, t) .and. Vnoadjust(i, j, k, l, t)<Vrent(i, j, k, l, t)) then
                            ...
                            if (anodes(i)-(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)<0) then
                                cchoice(i, j, k, l, t)=income(k, t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*P_exp(t-age+1)-thetanodes(l)*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+rborrow)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                                !write(*,*) "diff2", income(k, t)+(1+rborrow*P_exp(t-age+1)**)*anodes(i)+Dnodes(j)*(1-delta)*P_exp(t-age+1)-thetanodes(l)*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+rborrow*P_exp(t-age+1)**)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)-(income(k, t)+(1+rborrow*P_exp(t-age+1)**(-0.0))*anodes(i)+Dnodes(j)*(1-delta)*P_exp(t-age+1)-thetanodes(l)*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+rborrow*P_exp(t-age+1)**(-0.0))*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1))
                            else
                                cchoice(i, j, k, l, t)=income(k, t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*P_exp(t-age+1)-thetanodes(l)*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+r)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                            end if
                            rentalindicator(i, j, k, l, t)=0
                            choiceindicator(i, j, k, l, t)=2
                        else
                            ...
                            Dchoice(i, j, k, l, t)=Dchoicerent(i, j, k, l, t)
                            if (anodes(i)-(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)<0) then
                                cchoice(i, j, k, l, t)=income(k, t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-r_rental*Dchoice(i, j, k, l, t)*exp(rentelasticity*hpnodes(l))-achoice(i, j, k, l, t)-(1+rborrow)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                                !write(*,*) "diff3", income(k, t)+(1+rborrow*P_exp(t-age+1)**)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-r_rental*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+rborrow*P_exp(t-age+1)**)*(1-thetanodes(l))*(1-F)*Dnodes(j)*P_exp(t-age+1) - (income(k, t)+(1+rborrow*P_exp(t-age+1)**(-0.0))*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-r_rental*Dchoice(i, j, k, l, t)*P_exp(t-age+1)-achoice(i, j, k, l, t)-(1+rborrow*P_exp(t-age+1)**(-0.0))*(1-thetanodes(l))*(1-F)*Dnodes(j)*P_exp(t-age+1))
                            else
                                cchoice(i, j, k, l, t)=income(k, t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*P_exp(t-age+1)-r_rental*Dchoice(i, j, k, l, t)*exp(rentelasticity*hpnodes(l))-achoice(i, j, k, l, t)-(1+r)*(1-thetanodes(l))*Dnodes(j)*P_exp(t-age+1)
                            end if
                            !write(*,*) l,(1+rborrow*P_exp(t-age+1)**)
                            rentalindicator(i, j, k, l, t)=1
                            choiceindicator(i, j, k, l, t)=3
                        end if
                       ! write(*,*) P_exp(t-age+1), (1+rborrow*P_exp(t-age+1)**), income(6, l, 1)
                    end do
                end do
            end do
        end do
...
!$OMP END PARALLEL
        ...
        if (t-age == totalbubblelength) then
            EVelig(:,:,:,:,t) = max(Vrent(:,:,:,:, t), max(Vnoadjust(:,:,:,:, t), Vadjust(:,:,:,:, t)))
            EVinelig(:,:,:,:,t) = max(Vrent(:,:,:,:, t), max(Vnoadjust(:,:,:,:, t), Vinelig(:,:,:,:, t)))
        else
            EVelig(:,:,:,:,t) = max(Velig(:,:,:,:, t), max(Vnoadjust(:,:,:,:, t), Vadjust(:,:,:,:, t))) 
            EVinelig(:,:,:,:,t) = max(Vinelig(:,:,:,:, t), max(Vnoadjust(:,:,:,:, t), Vadjust(:,:,:,:, t))) 
        end if
...
DEALLOCATE(ax, bx, cx, adjust)
end subroutine solveanticipationproblem
...
Dprime=Dcurrent
aprimeholder=aprime
aprime=aprime+(1-thetaholder)*Dcurrent*(P_exp(t-age+1+1)-P_exp(t-age+1))

if (aprime>anodes(agridsize)) then
aprime=anodes(agridsize)
end if

if (P_exp(t-age+1+1)-P_exp(t-age+1) /= 0) then
    hpholder=gd(t-age+1)*(1-rho**(t))/(1-rho)
end if
...
aprimeholder=aprime
aprime=max(borrowconstraint,
           aprime+(1-thetaholder)*Dcurrent*(P_exp(t-age+1+1)-P_exp(t-age+1))   
if (aprime>anodes(agridsize)) then
aprime=anodes(agridsize)
end if
