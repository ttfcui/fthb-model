module share
    ...
    integer, parameter :: agridsize = 90 ! size of asset grid
    integer, parameter :: Dgridsize = 30 ! size of durable grid
    ...
    REAL(8), parameter :: numhouseholds=50000    !Size of household panel
...
OPEN (UNIT=901, FILE="momentsgridsearch.txt", STATUS="OLD", ACTION="WRITE", POSITION="REWIND")
...
! death probabilityes (starting at age 60 or 65)
    ...
    !retirement regression needs to first run matlab program simulateearningDProcess
    do j=1,zgridsize
...
OPEN (UNIT=111, FILE="lifecyclemoments.txt", STATUS="OLD", ACTION="WRITE", POSITION="REWIND")
...
!OPEN (UNIT=12, FILE="allhouseholdstates.txt", STATUS="OLD", ACTION="WRITE", POSITION="REWIND")
                    ...
                    do l=1,hpgridsize
                        ...
                        if (Vadjust(i,j,k,l,t)<Vnoadjust(i,j,k,l,t) .and. Vadjust(i,j,k,l,t)<Vrent(i,j,k,l,t)) then  ! since V = - V from minimization
                            ...
                            if (anodes(i)-(1-theta)*Dnodes(j)*exp(hpnodes(l))<0) then
                                cchoice(i,j,k,l,t)=income(k,t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*exp(hpnodes(l))-theta*Dchoice(i,j,k,l,t)*exp(hpnodes(l))-achoice(i,j,k,l,t)-(1+rborrow)*(1-theta)*(1-F)*Dnodes(j)*exp(hpnodes(l))
                                ...
                                cchoice(i,j,k,l,t)=income(k,t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*exp(hpnodes(l))-theta*Dchoice(i,j,k,l,t)*exp(hpnodes(l))-achoice(i,j,k,l,t)-(1+r)*(1-theta)*Dnodes(j)*(1-F)*exp(hpnodes(l))
                            end if
                        ...
                        elseif (Vnoadjust(i,j,k,l,t)<Vadjust(i,j,k,l,t) .and. Vnoadjust(i,j,k,l,t)<Vrent(i,j,k,l,t)) then
                            ...
                            if (anodes(i)-(1-theta)*Dnodes(j)*exp(hpnodes(l))<0) then
                                cchoice(i,j,k,l,t)=income(k,t)+(1+rborrow)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*exp(hpnodes(l))-r_rental_t(t)*Dchoice(i,j,k,l,t)*exp(rentelasticity*hpnodes(l))-achoice(i,j,k,l,t)-(1+rborrow)*(1-theta)*(1-F)*Dnodes(j)*exp(hpnodes(l))
                                ...
                                cchoice(i,j,k,l,t)=income(k,t)+(1+r)*anodes(i)+Dnodes(j)*(1-delta)*(1-F)*exp(hpnodes(l))-r_rental_t(t)*Dchoice(i,j,k,l,t)*exp(rentelasticity*hpnodes(l))-achoice(i,j,k,l,t)-(1+r)*(1-theta)*(1-F)*Dnodes(j)*exp(hpnodes(l))
                            end if
...
if (aprime>=borrowconstraint .AND. aprime<=anodes(agridsize) .AND. Dprime>=Dnodes(1) .AND. Dprime<=Dnodes(Dgridsize) .AND. Dcurrent>=0) then
    ...
    if ((a-D*(1-theta)*exp(hpholder))<0) then
        consumption=(currentincome+(1+rborrow)*a+D*(1-F)*(1-delta)*exp(hpholder)-theta*Dcurrent*exp(hpholder)-aprimeholder-(1+rborrow)*(1-theta)*D*(1-F)*exp(hpholder))
    else
        consumption=(currentincome+(1+r)*a+D*(1-F)*(1-delta)*exp(hpholder)-theta*Dcurrent*exp(hpholder)-aprimeholder-(1+r)*(1-theta)*D*(1-F)*exp(hpholder))
    end if
...
if (aprime>=borrowconstraint .AND. aprime<=anodes(agridsize) .AND. Dprime>=Dnodes(1) .AND. Dprime<=Dnodes(Dgridsize) .AND. Dcurrent>=0) then
    ...
    if ((a-D*(1-theta)*exp(hpholder))<0) then
        consumption=(currentincome+(1+rborrow)*a+D*(1-F)*(1-delta)*exp(hpholder)-r_rental_t(t)*Dcurrent*exp(rentelasticity*hpholder)-aprimeholder-(1+rborrow)*(1-theta)*D*(1-F)*exp(hpholder))
    else
        consumption=(currentincome+(1+r)*a+D*(1-F)*(1-delta)*exp(hpholder)-r_rental_t(t)*Dcurrent*exp(rentelasticity*hpholder)-aprimeholder-(1+r)*(1-theta)*D*(1-F)*exp(hpholder))
    end if
    !need to figure out Vadjust and Vnoadjust at offgrid points tomorrow.  Find nearest grid points and linearly interpolate.
    if (consumption>0) then
        if (elasticity .ne. 1) then
...
!BL
	...
	do
		...
		!write(*,*) stateholder
			...
			iter=iter+1
			if (ytry >= ysave) then
				p(:,:)=0.5_DP*(p(:,:)+DPread(p(ilo,:),1,size(p,1)))
				do i=1,ndim+1
