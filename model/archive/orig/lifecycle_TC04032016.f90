...
! TODO: loop over some (a, D) pairs here
...
!$OMP PARALLEL DO
    do i=1,numhouseholds
        ...
        ! Starts the FTHB policy timer.
            if ((fthb_flag(i,1) /= Tdie+1) .and. (fthb_flag(i,5) == Tdie+1)) then
                fthb_flag(i,5) = t
            end if
        ! Track agents again as FTHBs after three consecutive years of renting
            if ((t >= fthb_flag(i, 5) + (EligYrs - 1)) .and. (t <= Tretire)) then
            ...
            fthb_flag(i,5) = Tdie+1
        ! Indication of new house purchase
            if (choiceind(i,t) == 1.0) then
                numtrans(i) = 1
                housingflow(i,t)=newhouseholdstate(i,2)
                ! Record stats for agent when they buy their first house
...
write(*,*) "fracown", sum(numowntotal)/(sum(numowntotal)+sum(numrent))  !Target 69% from SCF 98 (luengo-prado and diaz)
...
fracown= sum(numowntotal)/(sum(numowntotal)+sum(numrent))

diffmoments1=log(.69/(fracown))**2
...
r_rental=r_rental+.018*(.69-fracown)
end if
...
currenthouseholdstate(:, 5) = t
do i = 1, numhouseholds
    if ((householdresultsholder(i, t, 10)<t) .and. &
        (t < householdresultsholder(i, t, 11) + EligYrs)) then
        currenthouseholdstate(i, 6) = 0 ! Not FTHB status
        newhouseholdstate(i, 6) = 0
        currenthouseholdstate(i, 4) = 2
    else
        currenthouseholdstate(i, 6) = 1 ! FTHB status (no house or 3 years since ownership)
...
!$OMP PARALLEL DO
    do i=1,numhouseholds
        ...
        ! Track agents again as FTHBs after three years of renting
            if ((tj >= fthb_flag(i,2) + (EligYrs-1)) .and. (t <= Tretire)) then
                newhouseholdstate(i,6)=1
                fthb_flag(i, 1) = Tdie+1
        ...
        if ((tj - t < 3) .or. ((tj - t > 8) .and. (tj - t <= 12))) then
            ...
            write(499,499) i, t, tj, newhouseholdstate(i,1), choiceind(i,tj), newhouseholdstate(i, 2), currenthouseholdstate(i,6), newhouseholdstate(i,6), fthb_flag(i,1), householdresultsholder(i, t, 10)
        end if
