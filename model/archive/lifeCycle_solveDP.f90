...
!$OMP DO
            ...
            !if ((i == 10) .AND. (j == 1) .AND. (k == 7)) then
            !    write(*,*) mySimplex_rent(:, :, i, i2, j, k, l)
            !end if
            !from here
            !do p1=1,3
                !if (k== agridsize .or. t == Tdie) then
                !do p2=1,2
                !   pstartadjust(p1, p2, i, i2, j, k, l)=amoebaGrid(p2, p1, 1)*&
                !        wealthHolder(i,i2,j,k,t,l) !+ &
                !        !(1-maint*delta-dtau)*max(Dmin-Dnodes(j), 0.0)*exp(price(l)))
                !end do
                !else
                !    pstartadjust(p1, 1, i, i2, j, k, l)=amoebaGrid(1, p1, 1)*achoicerent(i, i2, j, k+1, t, l)
                !    pstartadjust(p1, 2, i, i2, j, k, l)=amoebaGrid(2, p1, 1)*dchoicerent(i, i2, j, k+1, t, l)
                !end if
                !ystartadjust(p1, i, i2, j, k, l)=valfuncrent(&
                !    pstartadjust(p1,:,i,i2,j,k,l), state(:, i, i2, j, k, l),&
                !    valfuncpol(2,:))
...
! %>
            ...
            ! %< Set up another 2-simplex to see if local minimum not global
            !do p1=1,3
            !    do p2=1,2
            !        pstartadjust(p1, p2, i, i2, j, k, l)=amoebaGrid(p2, p1, 2) * max(&
            !            wealthHolder(i,i2,j,k,t,l), (1+F/2.0)*Dmin*exp(price(l)) + 1e-2)
                    !should be this
                    !pstartadjust(p1, p2, i, i2, j, k, l)=amoebaGrid(p2, p1, 1) * wealthHolder(i,i2,j,k,t,l)
            !    end do
            ...
            !            ystartadjust(:,i,i2,j,k,l), ftol, valfuncpol(1,:), &
            !            valfuncadjust)
            !if (ystartadjust(1, i, i2, j, k, l)<Vadjust(i, i2, j, k, t,l)) then  !(again we're minimizing)
            !    Vadjust(i, i2, j, k, t,l)=ystartadjust(1, i, i2, j, k, l)
            !    achoiceadjust(i, i2, j, k, t,l)=min(max(pstartadjust(1, 1, i, i2, j, k, l),borrowconstraint),amax)
            !    Dchoiceadjust(i, i2, j, k, t,l)=min(max(pstartadjust(1, 2, i, i2, j, k, l),0.0),Dmax)
            !end if
            ...
            !cchoiceadjust(i,i2,j,k,t,l)=wealthHolder(i,i2,j,k,t,l) - costholder(i,i2,j,k,t,l) - &
            !    achoiceadjust(i,i2,j,k,t,l) &
            !    - (1-polLevel(3))*theta*Dchoiceadjust(i,i2,j,k,t,l)*(1+F/theta)&
            !    &*exp(price(l))/exp(downflag*transferholder(2,i,i2,j,k,t,l)/(1-polLevel(3))*theta) &
            !    + (1.0-downflag)*transferholder(1,i,i2,j,k,t,l) - polLevel(4) &
            !    - scrapped*((1-maint*delta-dtau)*Dnodes(j)*exp(price(l)))
                
            !else
            !cchoiceadjust(i,i2,j,k,t,l)=wealthHolder(i,i2,j,k,t,l) - costholder(i,i2,j,k,t,l) - &
            !    scrapped*((1-maint*delta-dtau)*Dnodes(j)*exp(price(l)) - scrapvalue) - (&
            !    theta*Dchoiceadjust(i,i2,j,k,t,l)*exp(price(l))&
            !    + achoiceadjust(i,i2,j,k,t,l))
            end if
...
!$OMP END PARALLEL
    ...
    if (noshock .EQV. .TRUE.) call output_vfuncs(l+1, EV, Vadjust, Vrent, Vnoadjust, achoice, dchoiceadjust, dchoice, cchoiceadjust, cchoice)
    ...
    subroutine output_vfuncs(period, EV, Vadjust, Vrent, Vnoadjust, achoice, dadjchoice, dchoice, cadjchoice, cchoice)
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ! %<
        !
            ...
            do i2=2, 2
              do j=1, Dgridsize/2, 4
                do k=1, agridsize
                    ...
                    t2=t+1
                    u2=u+1
                        !call computempc(anodes(k), Dnodes(j), k2, l2, t2, mpc_pol(i, j, k, 33, l))
                        write(u, '(14F40.12)') hpnodes(l), znodes(i), Dnodes(j), anodes(k), EV(i, i2, j, k, t, l), Vadjust(i, i2, j, k, t, l), Vrent(i, i2, j, k, t, l), Vnoadjust(i, i2, j, k, t, l), achoice(i, i2, j, k, t, l), dadjchoice(i, i2, j, k, t, l), dchoice(i, i2, j, k, t, l), cadjchoice(i, i2, j, k, t, l),  cchoice(i, i2, j, k, t, l),  income(i, t) !, mpc_pol(i, j, k, t, l)
                        write(u2, '(14F40.12)') hpnodes(l), znodes(i), Dnodes(j), anodes(k), EV(i, i2, j, k, t2, l), Vadjust(i, i2, j, k, t2, l), Vrent(i, i2, j, k, t2, l), Vnoadjust(i, i2, j, k, t2, l), achoice(i, i2, j, k, t2, l), dadjchoice(i, i2, j, k, t2, l), dchoice(i, i2, j, k, t2, l), cadjchoice(i, i2, j, k, t2, l),  cchoice(i, i2, j, k, t2, l),  income(i, t2) !, mpc_pol(i, j, k, t2, l)
                        end do
                    end do
                end do
            end do
      ...
      do i=1, zgridsize, 2
          do i2=2, 2
              do k=6, agridsize, 6
                  ...
                  j = 1  ! Renters
                      !call computempc(anodes(k), Dnodes(j), k2, l2, t2, mpc_pol(i, j, k, 33, l))
                      write(u, '(14F40.12)') hpnodes(l), znodes(i), anodes(k), real(t), EV(i, i2, j, k, t, l), Vadjust(i, i2, j, k, t, l), Vrent(i, i2, j, k, t, l),Vnoadjust(i, i2, j, k, t, l),  achoice(i, i2, j, k, t, l), dadjchoice(i, i2, j, k, t, l), dchoice(i, i2, j, k, t, l), cadjchoice(i, i2, j, k, t, l),  cchoice(i, i2, j, k, t, l),  income(i, t) !, mpc_pol(i, j, k, t, l)
                      end do
                  end do
              end do
          end do
