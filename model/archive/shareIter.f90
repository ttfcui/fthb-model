module share
    ...
    REAL(8), parameter :: beta2= 0.950000 ! 0.9175 !Quarterly discount factor
    REAL(8), parameter :: e_of_s= 2 ! elasticity of substitution (unused)
    REAL(8), parameter :: r= 0.010000 ! return on safe asset
    REAL(8), parameter :: rborrow= 0.020000 ! Plus 0.8% mortgage premium
    REAL(8), parameter :: delta= 0.022000 !depreciation rate of durable  was .03
    REAL(8), parameter :: dtau= 0.010000 !Property tax on housing
    REAL(8), parameter :: tax1= 0.175000 !Income tax on labour
    REAL(8), parameter :: tax2=.15 ! Progressivity parameter on income tax
    REAL(8), parameter :: F= 0.060000 ! fixed cost (e.g. realtor fees)
    REAL(8), parameter :: F2= 0.240000 !0.00 ! quadratic trans cost term (like furniture?)
    REAL(8) :: Dmin= 1.100000 ! Minimum owned house size (not parameter to fit to D grid)
    ...
    REAL(8), parameter :: elasticity= 3.000000 !3.5 ! intertemporal elasticity of substitution
    REAL(8), parameter :: rentPrem= 0.010000 !0.015 ! Rental operation costs on top of user cost
    REAL(8), parameter :: rentPremRetire= rentPrem ! Different premium for retirement
    REAL(8), parameter :: ret_wealth= 0.030000 ! Lump-sum transfer at retirement, relative to labour income
    REAL(8), parameter :: psi= 1.000000 ! End-of-life bequest parameter
    REAL(8), parameter :: rentUtil= 1 !.90 ! Disutility of renting relative to owning
    ...
    integer, parameter :: agridsize= 100 
    integer, parameter :: Dgridsize= 45 

    ! Permanent income process
    REAL(8), parameter :: sigma_z= 0.210000 ! Standard deviation of productivity shocks
    REAL(8), parameter :: rho_z= 0.910000 ! Autocorrelation of AR(1) process
    ...
    REAL(8), parameter :: unempprob= 0.000000 ! Prob. of going into unemp. (deprecated)
    REAL(8), parameter :: movprob= 0.015000 ! Prob. of exogenous move for owners
    REAL(8), parameter :: movprobR= 0.000000 ! Prob. of exogenous home purchase for renters (deprecated)
    REAL(8), parameter :: unemptrans= -0.05  ! Unemploymeny benefits (deprecated)

    !House price subsidy shocks and policy settings
    integer, parameter :: hptransLength=3 ! Total number of years in transition
    ...
    REAL(8), parameter :: Eta_transfer= 0 ! Eta but for monetary transfers
    ...
    REAL(8), parameter :: rebatedflag=0  ! Transaction costs eebated
    ...
    LOGICAL, parameter :: ge_start=.TRUE.!.FALSE.  
    ! If TRUE, solves the model in PE given GE steady-state
    LOGICAL, parameter :: pe_start=.FALSE.!.TRUE.  
    ...
    LOGICAL, parameter :: loadpricepath=.TRUE.!.FALSE. 
    ...
    LOGICAL, parameter :: no_lifeinc=.FALSE.!.TRUE.!.FALSE.  
    ...
    LOGICAL, parameter :: steady_state_block = .FALSE.
    ...
    subroutine share_arrays
        ...
        !Work in progress
        !amoebaGrid(:,:,1)= RESHAPE( (/ 0.95, 0.01, .50, 0.01, 0.25, 0.50 /), &
        !                            shape(amoebaGrid(:,:,1) ))
