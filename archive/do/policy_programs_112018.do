...
program define gen_groupings
        ...
        count if inlist(model, "FTHB Tax Credit", "experiment_monetary_nodown")
	if `r(N)' == 0 {
	    preserve
	...
	_pctile income_val if marginal==0 & inlist(model, "FTHB Tax Credit", "experiment_monetary_nodown"), pe(1)
	local lev1 = `r(r1)'

	* All working-age income dist
	capture preserve
	import delimited "fthb/fthb_experiment_monetary_nodown.txt", delim(" ", collapse) clear
	rename (v3 v10 v14) (age assets income_val)
...
program define relabel_models
	...
	replace model = "FTHB Tax Credit" if model=="experiment_monetary_nodown"
	replace model = "CARS Program" if model=="experiment_CARS"
	replace model = "CARS Program, Fixed Size" if model=="experiment_CARS_fixed"
	replace model = "FTHB Bridge Loan" if model=="experiment_monetary"
	replace model = "Unconditional \$8K Transfer" if model=="experiment_monetary_KV"
	replace model = "\$2K FTHB Tax Credit" if model=="experiment_FTHBSize_0.03"
	replace model = "\$2K FTHB Bridge Loan" if model=="experiment_FTHBonDownSize_0.03"
...
local rentprem = 0.00966
local cardep = 0.125
local housedep = 0.022
local pricecoef = 0.173
...
program define cost_benefit_durables
	...
	collapse (mean) durables_diff0 investment_diff* (count) id `ifquery', by(model)
	forv i=1/5 {
	    ...
	    replace npv_`i' = npv_`i'/0.12 if inlist(model, "FTHB Tax Credit", "FTHB Bridge Loan")
	...
	replace durables_diff0 = durables_diff0/0.12 if inlist(model, "FTHB Tax Credit", "FTHB Bridge Loan")
	replace durables_diff0 = durables_diff0/0.12*(id/2.4e5) if inlist(model, "Unconditional $8K Transfer")
	replace durables_diff0 = durables_diff0/0.06 if inlist(model, "CARS Program")
	replace durables_diff0 = durables_diff0/0.03 if regexm(model, "2K") == 1
...
program define dataprep_takers
    ...
    * policy takers but also changed their extensive margin
    ...
    preserve
    tempfile denoms
    count if model=="FTHB Tax Credit" & marginal == 0
    local denom = `r(N)'
    keep if decomp == 1
...
* Merging policy taker and target pop files

	tempfile master_main
	save `master_main'

	indiv_assembly monetary monetary_nodown monetary_KV CARS CARS_fixed ///
	     FTHBSize_0.03 FTHBonDownSize_0.03
	relabel_models
	replace marginal = 0 if inframarg == 1
	drop inframarg
	...
	tab infra_groups if model=="FTHB Tax Credit"
	tab infra_groups if model=="FTHB Tax Credit" & marginal==0
...
program define timing_tables
    ...
    decomp_table fthb/fthbvcars_timing.tex ///
        if inlist(model, "FTHB Tax Credit", "CARS Program"), tabvar(`tabvar')
...
* Bang-for-buck estimates for policy based on stimulated spending/cost
	...
	cost_benefit_durables if infra_groups <= 4, discount(1)
	local discount = 1/1.03
	cost_benefit_durables if infra_groups <= 4, discount(`discount')
	local discount = 1/1.1
	cost_benefit_durables if infra_groups <= 4, discount(`discount')
end /* %> */
...
timing_tables if model=="FTHB Bridge Loan", tabvar(infra_groups)
extensive_tables if model=="FTHB Bridge Loan", tabvar(infra_groups) fthbtrans(14671) carstrans(29699)
...
timing_tables if model=="FTHB Tax Credit", tabvar(infra_groups)
extensive_tables if model=="FTHB Tax Credit", tabvar(infra_groups) fthbtrans(14671) carstrans(29699)
...
* Bar plot of policy group decompositions
	...
	gen id2 = .  // No poorest/no assets stimulated
	...
	label def model_lab 1 "$2K FTHBTC" 2 "CARS Program" 3 "$8K Bridge Loan" 5 "$8K K-V Transfer" ///
	    4 "$8K FTHBTC", replace
	display "`rbar_call'"
	#delimit ;
	twoway `rbar_call' ||, ysc(reverse) ylab(,angle(0) val labs(small)) yti("") 
	    xti("Ratio of HH to Inframarginal Policy Takers") $graphopts
...
program define fig_propplots
  ...
  /* Propensity Plots */
    ...
    save `modelprob'
    use `main', clear
    keep if model=="FTHB Bridge Loan"
    ...
    graph bar id*, over(infra_groups,  ///
       ...
       legend(label(1 "No Policy") label(2 "$8K Bridge Loan")
...
/* Pullforward distribution degeneracy */
    ...
    pullf_dist infra_incval_tiers if inlist(model, "FTHB Bridge Loan", "FTHB Tax Credit"), ///
        compgroup(2 3) label1("Poor HH") label2("Non-Poor HH") name(hist_1)

    pullf_dist infra_incval_tiers if inlist(model, "CARS Program", "FTHB Tax Credit"), ///
    ...
    pullf_dist infra_asset_tiers if inlist(model, "FTHB Bridge Loan", "FTHB Tax Credit"), ///
...
local thres1 = .5308*67.250 // inframarginal tail
local thres2 = .8928*67.250 // median
binscatter durlevels_diff3 income_val if model=="FTHB Tax Credit", ///
    by(adjdummy) xline(`thres1' `thres2') rd(`thres2') xq(adjbins) `legendlab' name(fthb_bs, replace)
binscatter durlevels_diff3 income_val if model=="CARS Program", ///
    by(adjdummy) xline(`thres1' `thres2') rd(`thres2') xq(adjbins) `legendlab' name(cars_bs, replace)
binscatter durlevels_diff3 income_val if model=="FTHB Bridge Loan", ///
    by(adjdummy) xline(`thres1' `thres2') rd(`thres2') xq(adjbins) `legendlab' name(fthbbl_bs, replace)
...
extensive_tables, tabvar(model) fthbtrans(14671) carstrans(29699)

dataprep_renters
fig_barplots if inlist(model, "\$2K FTHB Bridge Loan", "CARS Program, Fixed Size")
fig_propplots if inlist(model, "FTHB Tax Credit", "FTHB Bridge Loan", "Unconditional \$8K Transfer")
stimulus_prep
